...
def getLangchainResponse(langchainModel, text, modelName, chatId):
    ...
    try:
        # If the user has changed the model, we need to reinitialize the workflow
...
def getLangchainResponseMergestack(modelName, text):
    try:
        ...
        print(response)
        # returning the response
        return ResponseBuilder().setSuccess(True).setMessage("Response Generated Successfully").setData(encrypt(response)).setStatusCode(200).build()
